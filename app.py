...
class YTDL_GUI(tb.Window):
    ...
    def create_widgets(self):
        # Root grid: 3 cols, center ctrl frame
        ...
        # Save to folder row with inner frame
        ...
        for txt, style, cmd in [
            ...
            tb.Button(action, text=txt, command=cmd, bootstyle=style).pack(side=LEFT, padx=8)

        # Download queue below
    ...
    def add_to_queue(self):
        ...
        job = {"id": job_id, "url": url, "fmt": fmt, "outdir": outdir, "status": "Queued", "progress": 0}
    ...
    def download_task(self, job):
        def hook(d):
            if d.get('status') == 'downloading':
                ...
                if total:
                    ...
                    self.tree.set(job['id'], 'Progress', f"{percent}%")
        ...
        try:
            ...
            job['status'] = 'Error'
            self.tree.set(job['id'], 'Status', 'Error')
